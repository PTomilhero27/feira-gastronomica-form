 ...
 *
 * Ajustes pedidos:
 * - Começa SEM nenhuma categoria (estado vazio com texto).
 * - Só deixa avançar se tiver:
 *   - pelo menos 1 categoria
 *   - e cada categoria tiver pelo menos 1 produto
 * - Visual coerente com o Step 1 (header “bege/laranja”, botões laranja, card com divisória).
 * - Mantém drag sem libs externas.
 */
export function StallStep2Menu({
  ...
  const [prodDrafts, setProdDrafts] = React.useState<Array<{ name: string; price: string }>>([
    { name: '', price: '' },
  ])
  ...
  function confirmAddProducts() {
    ...
    const next = categories.map((c, i) => {
      ...
      return {
        ...
        products: [
          ...(c.products ?? []),
          ...prodDrafts.map((d) => ({ name: d.name.trim(), price: d.price.trim() })),
        ],
      }
    })
  ...
  function validateForNext(): { ok: true } | { ok: false; message: string } {
    if (!categories.length) {
      return { ok: false, message: 'Adicione pelo menos 1 categoria para continuar.' }
    }
    ...
    for (const c of categories) {
      ...
      if (!c.products?.length) {
        return { ok: false, message: `A categoria "${catName}" precisa ter pelo menos 1 produto.` }
      }
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
      {/* Header (coerente com Step 1) */}
      <div className="bg-orange-50 px-6 py-5">
        <h3 className="text-lg font-semibold text-zinc-900">Cardápio</h3>
                  ...
                  <div className="relative">
                    ...
                    {catMenuOpenIdx === catIdx ? (
                      <div className="absolute right-0 z-10 mt-2 w-56 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-lg">
                        <MenuItem icon={<Pencil className="h-4 w-4" />} label="Editar categoria" onClick={() => openEditCategory(catIdx)} />
                        <MenuItem icon={<PackagePlus className="h-4 w-4" />} label="Adicionar produto" onClick={() => openAddProducts(catIdx)} />
                        <MenuItem
