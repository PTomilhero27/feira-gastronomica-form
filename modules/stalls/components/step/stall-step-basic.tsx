 ...
 * Responsabilidade:
 * - Coletar dados obrigatórios da barraca
 *
 * Ajustes:
 * - Tipo da barraca agora tem 3 opções: OPEN | CLOSED | TRAILER
 * - Se tipo = TRAILER:
 *   - tamanho é desmarcado automaticamente
 *   - opções de tamanho ficam desabilitadas
 */
export function StallStep1Basic({
  ...
  value: {
    ...
    stallSize: 'SIZE_2X2' | 'SIZE_3X3' | 'SIZE_3X6' | '' // ✅ permite limpar
    teamQty: number
    stallType: 'OPEN' | 'CLOSED' | 'TRAILER' | '' // ✅ adiciona TRAILER
  }
  ...
  /**
   * ✅ Ao sair de "Outro", limpa o texto digitado.
   */
  React.useEffect(() => {
  ...
  /**
   * ✅ Se escolher TRAILER, desmarca tamanho automaticamente.
   * Evita inconsistência (trailer não depende de 2x2/3x3/3x6).
   */
  React.useEffect(() => {
  ...
  function setDigitsNumber<K extends 'machinesQty' | 'teamQty'>(key: K, raw: string) {
    const digits = (raw ?? '').replace(/\D/g, '')
    const n = digits === '' ? 0 : Number(digits)
    onChange({ ...value, [key]: n } as any)
  }
  ...
  function validateAndNext() {
    ...
    if (!value.stallType) {
      toast.warning({
        ...
        subtitle: 'Selecione se é aberta, fechada ou trailer.',
      })
      return
    }

    // ✅ tamanho obrigatório apenas quando NÃO for trailer
    if (!isTrailer && !value.stallSize) {
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          ...
          <Field className="md:col-span-1" label="Quantidade de máquinas *">
            <Input
              ...
              onChange={(e) => setDigitsNumberClamped('machinesQty', e.target.value, 1, 5)}
              placeholder="Ex.: 2"
              max={5}
            />
          ...
          {/* ✅ Tamanho da barraca (desabilita se for trailer) */}
          <Field className="md:col-span-2" label="Tamanho da barraca *">
            <div
              className={[
                'grid grid-cols-1 gap-3 sm:grid-cols-3',
                isTrailer ? 'pointer-events-none opacity-50' : '',
              ].join(' ')}
            >
              <PickCard
                title="2m × 2m"
      ...
      <div className="flex items-start gap-3">
        <div
          className={[
            ...
            selected ? 'border-orange-500 bg-orange-500' : 'border-zinc-300 bg-white',
          ].join(' ')}
        />
